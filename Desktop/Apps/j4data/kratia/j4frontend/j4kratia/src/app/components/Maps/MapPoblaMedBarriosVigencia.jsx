...
export default function MapPoblaBarrios({vigencia, geojson}) {
   
    const position = [6.2982733, -75.5459204];
    const MyDataPoblaBarrio = () => {
        const map = useMap()
        ...
        const geojsonObject = L.geoJSON(geojson, {
           ...
           <div class="card-body">
                      <h5 class="card-title">` + features.properties.NOMBREBARRIO + `</h5>
                      <h6 class="card-subtitle mb-2 text-muted">  ${features.properties.CODCOMUNA} -  ${features.properties.NOMBRECOMUNA}</h6>
             <p class="card-text">
             <p  class="text-muted"  ;">Código Territorio <br> <b>`+ (features.properties.CODBARRIO) + `</b> </p>
             <table class="table table-hover table-inverse table-responsive">
                <tbody>
                  ...
                  <tr>  
                    <td>Hombres</td>
                    <td>` + formatter.format(features.properties.TOTALHOMBRE) + `</td>
                  </tr>
                  <tr>
                     <td>Mujeres</td>
                      <td>` + formatter.format(features.properties.TOTALMUJER) + `</td>
                  </tr>
                </tbody>
              </table>
            </div>
          </div>`
                let ToolTip = `${features.properties.NOMBREBARRIO} <br/> Total: ${formatter.format(features.properties.TOTAL)}<br/> 
              Mujeres : ${formatter.format(features.properties.TOTALMUJER)} <br/> 
              Hombres : ${formatter.format(features.properties.TOTALHOMBRE)} 
              `
        ...
        info.update = function (props) {
            this._div.innerHTML = ` <h4>Población por Barrios <br />${vigencia}</h4>`
        };
